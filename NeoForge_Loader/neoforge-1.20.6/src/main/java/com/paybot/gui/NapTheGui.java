        ...
        for (int i = 0; i < telcos.length; i++) {
            ...
            if (customLoreEnabled) {
                ...
                if (rawLore != null && !rawLore.isEmpty()) {
                    lore = com.paybot.utils.CustomLoreFormatter.formatLore(player, rawLore, 0, "");
                }
            }
    ...
    public static void openDenomGui(ServerPlayer player, String telco) {
        ...
        for (int i = 0; i < denoms.length; i++) {
            ...
            String displayName = customName.isEmpty()
                    ...
                    : com.paybot.utils.CustomLoreFormatter.formatName(player, customName, denom, coinAmt);

            if (customLoreEnabled) {
                List<String> rawLore = mod.getConfig().getStringList("custom-lore.card." + denom);
                if (rawLore != null && !rawLore.isEmpty()) {
                    loreList = com.paybot.utils.CustomLoreFormatter.formatLore(player, rawLore, denom, coinAmt);
                }
            }
