...
public class NeoForgeVersionAdapterModern implements VersionAdapter {
    ...
    // Dynamic ResourceLocation class — may not exist in MC 1.21.11+ (class removed/moved)
    ...
    private synchronized void ensureInitialized() {
        ...
        LOGGER.info("[NeoForgeModern] Tra registry — custom_name={}, lore={}, custom_data={}",
        ...
        if (anchor == null) {
            LOGGER.error("[NeoForgeModern] Không lấy được BẤT KỲ DataComponentType nào qua registry.");
            PayBotDebug.logSwallowed("NeoForgeVersionAdapterModern.ensureInitialized: không có anchor", null);
            return;
        }
    ...
    private void resolveRegistry() {
        ...
        try {
            ...
            LOGGER.error("[NeoForgeModern] Không đọc được field của BuiltInRegistries: {}", t.getMessage());
            return;
        }
    ...
    public MapItemSavedData getMapSavedData(ItemStack mapItem, ServerLevel world) {
        ...
        try {
            ...
            PayBotDebug.logSwallowed("NeoForgeVersionAdapterModern.getMapSavedData", t);
        }
        return null;
    }
