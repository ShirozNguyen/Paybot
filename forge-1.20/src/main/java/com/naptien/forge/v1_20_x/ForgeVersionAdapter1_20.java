package com.naptien.forge.v1_20_x;
...
/**
 * ForgeVersionAdapter1_20 — v5.5.5 Part 44 [ĐÃ HARDEN]
 *
 * Adapter NBT thô cho nhánh Minecraft 1.20.0 - 1.20.4 (module forge-legacy/, biên dịch
 ...
 *     KHÔNG tồn tại ở các bản MC cũ nhất (1.20.0 - 1.20.4); thay bằng thao tác CompoundTag thủ
...
public class ForgeVersionAdapter1_20 implements VersionAdapter {
    private static final Logger LOGGER = LoggerFactory.getLogger("PayBot-Forge-Adapter1_20_x");
    ...
    private void setName(ItemStack stack, Component nameComponent) {
        try {
            ...
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_20.setName: setHoverName() lỗi", t);
        }
    ...
    private void setLore(ItemStack stack, List<String> lore) {
        CompoundTag displayTag = getOrCreateDisplayTag(stack);
        if (displayTag == null) {
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_20.setLore: không lấy được display tag — bỏ qua set lore.", null);
            return;
        }
    ...
    public void lockMap(MapItemSavedData state) {
        ...
        if (lockedField != null) {
            try {
                ...
                PayBotDebug.logSwallowed("ForgeVersionAdapter1_20.lockMap: set field 'locked' thất bại", t);
            }
        }
        try {
            ...
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_20.lockMap: cả field lẫn method lock() đều không dùng được — "
    ...
    private static synchronized void initFieldReflection() {
        ...
        if (boolFields.size() == 1) {
            ...
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_20.initFieldReflection: có " + boolFields.size()
    ...
    private CompoundTag getOrCreateTag(ItemStack stack) {
        try {
            ...
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_20.getOrCreateTag", t);
            return null;
        }
    }
    ...
    private CompoundTag getTag(ItemStack stack) {
        try {
            ...
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_20.getTag", t);
            return null;
        }
    }
    ...
    private CompoundTag getOrCreateDisplayTag(ItemStack stack) {
        ...
        try {
            ...
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_20.getOrCreateDisplayTag", t);
            return null;
        }
    }
    ...
    private String safeComponentToJson(Component comp) {
        ...
        try {
            ...
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_20.safeComponentToJson", t);
