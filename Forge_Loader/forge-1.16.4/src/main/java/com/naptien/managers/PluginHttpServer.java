...
// v5.5.5 Part 72: Fix JsonObject iteration for older Gson compatibility
...
public class PluginHttpServer extends NanoHTTPD {
    ...
    public Response serve(IHTTPSession session) {
        ...
        if ("GET".equals(method) && "/api/ping".equals(uri)) {
            return ok("{\"ok\":true,\"version\":\"" + PayBotMod.getModVersion() + "\"}");
        }
    ...
    private Response handleSePay(IHTTPSession session, JsonObject data) {
        ...
        // đã dùng cho các endpoint khác trong chính file này.
        String configuredKey = mod.getConfig().getString("sepay.secret-key", "").trim();
        if (configuredKey.isEmpty()) {
            PayBotMod.LOGGER.warn("[SePay] IPN bị từ chối từ IP " + clientIp
                    + ": config \"sepay.secret-key\" chưa được cấu hình. Vui lòng đặt 1 khoá bí mật "
                    + "mạnh (trùng với API Key trên dashboard SePay) trong config.yml để bảo vệ endpoint này.");
            return err("sepay.secret-key chưa được cấu hình");
        }
        ...
        if (!receivedKey.equals(configuredKey)) {
            PayBotMod.LOGGER.warn("[SePay] IPN auth thất bại từ IP " + clientIp);
            PayBotDebug.logSwallowed("SePay IPN auth failed từ " + clientIp, null);
    ...
    private Response handleRewardPush(IHTTPSession session, JsonObject data) {
        ...
        mod.runOnMainThread(() -> {
            ServerPlayer player = mod.getServer().getPlayerList().getPlayerByName(fPlayer);
            if (player != null) {
