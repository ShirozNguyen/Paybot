// v5.5.5 Part 84: Use ClientboundSetTitlesPacket for Forge 1.16.x in forge-1.16.4
...
public class TestPaymentGui {
    ...
    public static void fakeApproveBank(ServerPlayer player, int amount) {
        ...
        try { if (!rawAmt.isEmpty()) rewardAmt = (int)(Double.parseDouble(rawAmt) * modeMulti); }
            ...
            // v5.5.5 Part 58 [cùng bug đã sửa ở TopupListGui.java Part 55]: TRƯỚC ĐÂY im lặng.
            // rewardAmt feed vào placeholder [amount] trong lệnh thưởng — lỗi config admin, tool
            // TEST-ONLY nên mức độ thấp, nhưng vẫn nên log để admin phát hiện ngay lúc đang test.
            PayBotMod.LOGGER.warn("[TestPaymentGui] denom-rewards-bank." + amount + ".amt=\"" + rawAmt
    ...
    public static void fakeApproveCard(ServerPlayer player, String telco, int denom) {
        ...
        try { if (!rawAmt.isEmpty()) rewardAmt = (int)(Double.parseDouble(rawAmt) * modeMulti); }
            ...
            // rewardAmt feed vào placeholder [amount] trong lệnh thưởng — lỗi config admin, tool
            // TEST-ONLY nên mức độ thấp, nhưng vẫn nên log để admin phát hiện ngay lúc đang test.
            PayBotMod.LOGGER.warn("[TestPaymentGui] denom-rewards-card." + denom + ".amt=\"" + rawAmt
                    + "\" không parse được — [amount] sẽ là 0 trong lệnh thưởng.");
        }
