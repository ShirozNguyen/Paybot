package com.naptien.forge.v1_19_x;
...
/**
 * ForgeVersionAdapter1_19 — v5.5.5 Part 44 [ĐÃ HARDEN]
 *
 * Adapter NBT thô cho nhánh Minecraft 1.19.x (module forge-legacy/, biên dịch
 * nhắm 1.20.1). Forge dùng tên Mojang trực tiếp cả lúc dev lẫn lúc chạy thật từ bản 1.17 — dải 1.17-1.20.1 tương đối ổn định; các bản rất cũ hơn (1.14-1.16) dùng cơ chế mapping cũ hơn nên nếu vẫn lỗi, hãy bật debug-mode và gửi log để kiểm tra thêm.
 *
 ...
 *     KHÔNG tồn tại ở các bản MC cũ nhất (1.19.x); thay bằng thao tác CompoundTag thủ
 ...
 */
public class ForgeVersionAdapter1_19 implements VersionAdapter {
    private static final Logger LOGGER = LoggerFactory.getLogger("PayBot-Forge-Adapter1_19_x");
    ...
    private void setName(ItemStack stack, Component nameComponent) {
        try {
            ...
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_19.setName: setHoverName() lỗi", t);
        }
    ...
    private void setLore(ItemStack stack, List<String> lore) {
        CompoundTag displayTag = getOrCreateDisplayTag(stack);
        if (displayTag == null) {
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_19.setLore: không lấy được display tag — bỏ qua set lore.", null);
            return;
        }
    ...
    public MapItemSavedData getMapSavedData(ItemStack mapItem, ServerLevel world) {
        ...
        try {
            ...
            LOGGER.error("[Forge1_19_x] getMapSavedData error: {}", t.getMessage());
        }
        return null;
    }
    ...
    public void lockMap(MapItemSavedData state) {
        ...
        if (lockedField != null) {
            try {
                ...
                PayBotDebug.logSwallowed("ForgeVersionAdapter1_19.lockMap: set field 'locked' thất bại", t);
            }
        }
        try {
            ...
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_19.lockMap: cả field lẫn method lock() đều không dùng được — "
    ...
    private static synchronized void initFieldReflection() {
        ...
        if (boolFields.size() == 1) {
            ...
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_19.initFieldReflection: có " + boolFields.size()
    ...
    private CompoundTag getOrCreateTag(ItemStack stack) {
        try {
            ...
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_19.getOrCreateTag", t);
            return null;
        }
    }
    ...
    private CompoundTag getTag(ItemStack stack) {
        try {
            ...
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_19.getTag", t);
            return null;
        }
    }
    ...
    private CompoundTag getOrCreateDisplayTag(ItemStack stack) {
        ...
        try {
            ...
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_19.getOrCreateDisplayTag", t);
            return null;
        }
    }
    ...
    private String safeComponentToJson(Component comp) {
        ...
        try {
            ...
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_19.safeComponentToJson", t);
