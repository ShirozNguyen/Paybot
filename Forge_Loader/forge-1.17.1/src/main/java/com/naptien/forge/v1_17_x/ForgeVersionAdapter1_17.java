package com.naptien.forge.v1_17_x;
...
/**
 * ForgeVersionAdapter1_17 — v5.5.5 Part 44 [ĐÃ HARDEN]
 *
 * Adapter NBT thô cho nhánh Minecraft 1.17.x (module forge-legacy/, biên dịch
 ...
 *     KHÔNG tồn tại ở các bản MC cũ nhất (1.17.x); thay bằng thao tác CompoundTag thủ
 ...
 */
public class ForgeVersionAdapter1_17 implements VersionAdapter {
    private static final Logger LOGGER = LoggerFactory.getLogger("PayBot-Forge-Adapter1_17_x");
    ...
    private void setName(ItemStack stack, Component nameComponent) {
        try {
            ...
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_17.setName: setHoverName() lỗi", t);
        }
    ...
    private void setLore(ItemStack stack, List<String> lore) {
        CompoundTag displayTag = getOrCreateDisplayTag(stack);
        if (displayTag == null) {
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_17.setLore: không lấy được display tag — bỏ qua set lore.", null);
            return;
        }
    ...
    public MapItemSavedData getMapSavedData(ItemStack mapItem, ServerLevel world) {
        ...
        try {
            ...
            LOGGER.error("[Forge1_17_x] getMapSavedData error: {}", t.getMessage());
        }
        return null;
    }
    ...
    public void lockMap(MapItemSavedData state) {
        ...
        if (lockedField != null) {
            try {
                ...
                PayBotDebug.logSwallowed("ForgeVersionAdapter1_17.lockMap: set field 'locked' thất bại", t);
            }
        }
        try {
            ...
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_17.lockMap: cả field lẫn method lock() đều không dùng được — "
    ...
    private static synchronized void initFieldReflection() {
        ...
        if (boolFields.size() == 1) {
            ...
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_17.initFieldReflection: có " + boolFields.size()
    ...
    private CompoundTag getOrCreateTag(ItemStack stack) {
        try {
            ...
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_17.getOrCreateTag", t);
            return null;
        }
    }
    ...
    private CompoundTag getTag(ItemStack stack) {
        try {
            ...
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_17.getTag", t);
            return null;
        }
    }
    ...
    private CompoundTag getOrCreateDisplayTag(ItemStack stack) {
        ...
        try {
            ...
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_17.getOrCreateDisplayTag", t);
            return null;
        }
    }
    ...
    private String safeComponentToJson(Component comp) {
        ...
        try {
            ...
            PayBotDebug.logSwallowed("ForgeVersionAdapter1_17.safeComponentToJson", t);
