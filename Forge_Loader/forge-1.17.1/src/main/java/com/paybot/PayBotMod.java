...
import com.paybot.log.LogSpamFilter;
...
public class PayBotMod {
    ...
    public static void init() {
        try {
            com.paybot.compat.LibraryDownloader.checkAndDownloadLibraries(FMLPaths.CONFIGDIR.get().toFile());
    ...
    // v5.5.5 [audit - BUG FIX]: giong het van de da fix o forge-1.19.x/1.20.x - thieu
    // hoan toan co che chat interception. QUAN TRONG: ServerChatEvent.java co it nhat
    // 3 chu ky khac nhau tuy version Forge (xac nhan qua javadoc chinh thuc): <=1.16.x
    // dung getMessage() tra String (khong co getRawText), ~1.18.x co them 2 tham so
    // filter (chat-report feature), 1.19+ dung getRawText(). Vi khong the xac nhan
    // 100% chinh xac chu ky cho tung version qua tai lieu (dac biet 1.17.1 khong tim
    // duoc javadoc truc tiep), dung REFLECTION de tu thich ung - chi phu thuoc
    // getPlayer()/setCanceled() la 2 method chac chan on dinh xuyen suot lich su
    // ServerChatEvent (ke thua tu Event va co trong moi javadoc da xac nhan).
    @SubscribeEvent
    public void onServerChat(ServerChatEvent event) {
        ServerPlayer sender = event.getPlayer();
        String text = getChatEventText(event);
        if (text == null) return; // khong tim duoc method lay text qua reflection - bo qua an toan
    ...
    /**
     * Lay noi dung text tho cua ServerChatEvent qua reflection, thu cac ten method
     * theo thu tu uu tien (moi nhat truoc): getRawText() (1.19+), getMessage() dang
     * String (<=1.16.x hoac ~1.18.x). Neu ket qua la Component (khong phai String),
     * goi .getString() de lay text hien thi.
     */
    ...
    private void onServerStart() {
        ...
        dataDir = FMLPaths.GAMEDIR.get().resolve("PayBot");
        ...
        LOGGER.info("[PayBot] ════════════════════════════════════════");
        LOGGER.info("[PayBot]   PayBot Forge v" + getModVersion() + " — Sẵn sàng!");
    ...
    private void onPlayerQuit(ServerPlayer player) {
        com.paybot.gui.GuiSession.remove(player.getUUID());
    ...
    private void checkEnforceSecureProfile() {
        try {
            Path propsPath = FMLPaths.GAMEDIR.get().resolve("server.properties");
    ...
    private void checkEnforceSecureProfileWarn(ServerPlayer player) {
        ...
        try {
            ...
            server.execute(() -> {
                ...
                if (secureProfile && onlineMode) {
                    player.sendMessage(new TextComponent("§c§l[PayBot] §r§cCẢNH BÁO: §fenforce-secure-profile=true + online-mode=true đang bật!"), net.minecraft.Util.NIL_UUID);
                    player.sendMessage(new TextComponent("§7Kết hợp này có thể gây xung đột với PayBot (middleware chat-relay, chữ ký chat)."), net.minecraft.Util.NIL_UUID);
                    player.sendMessage(new TextComponent("§7→ Vào server.properties và đặt §fenforce-secure-profile=§cfalse §7(khuyến nghị)"), net.minecraft.Util.NIL_UUID);
                    player.sendMessage(new TextComponent("§7hoặc tắt §fonline-mode §7(nếu server offline chủ ý)."), net.minecraft.Util.NIL_UUID);
                } else {
                    player.sendMessage(new TextComponent("§e[PayBot] §7Gợi ý: §fenforce-secure-profile=true §7đang bật. Khuyến nghị đặt §bfalse §ftrong server.properties §7để tránh rủi ro khi cài thêm proxy/mod chat."), net.minecraft.Util.NIL_UUID);
                }
            });
        } catch (Exception ignored) {}
    }
    ...
    private void cleanExpiredQRMapsOnJoin(ServerPlayer player) {
        ...
        for (int i = 0; i < inv.getContainerSize(); i++) {
            ...
            String invId = com.paybot.utils.ItemTagCompat.getInvoiceId(stack);
    ...
    public static void sendBotDisabledNotice(net.minecraft.commands.CommandSourceStack src) {
        ...
        Component line2 = new TextComponent("§7Nếu bạn muốn hỗ trợ thì ")
                .append(com.paybot.utils.ClickableTextHelper.makeOpenUrl(
                        "§a§nnhấn vào đây",
