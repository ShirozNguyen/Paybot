 ...
 *   - Nếu cả 5 lần POST đều không thành công -> Chuyển sang phương thức GET, cũng thử 5 lần với backoff riêng.
 *   - Toàn bộ lỗi (trước đây chỉ log ở mức DEBUG — hiệu quả im lặng vì cấu hình log mặc định không in
 *     DEBUG) giờ được route qua PayBotDebug, bật/tắt bằng đúng config "debug-mode" của PayBot (độc lập
 *     với cấu hình logging framework của server).
 *
...
public class DirectCardSubmitHandler {
    ...
    /** Backoff (ms) theo thứ tự lần thử 1..5 — tăng dần rồi giữ trần ở 8s. */
    ...
    public static JsonObject submitDirectly(String siteUrl, String partnerId, String partnerKey,
        ...
        LOGGER.warn("[DirectCardSubmit] ⚠️ Cả {} lần thử POST đều thất bại (lần cuối: {}). Chuyển sang phương thức GET...",
        ...
        LOGGER.error("[DirectCardSubmit] ❌ Gửi thẻ thất bại hoàn toàn sau {} lần POST và {} lần GET tới {}. Lỗi cuối: {}{}",
                ...
                com.naptien.utils.PayBotDebug.isEnabled() ? "" : " (bật \"debug-mode: true\" trong config để xem chi tiết từng lần thử)");
        return null;
    }
    ...
    private static AttemptResult tryPostSubmit(String siteUrl, String partnerId, String partnerKey,
                                             String telco, int amount, String cardCode, String cardSerial, String requestId,
        ...
        try {
            ...
            com.naptien.utils.PayBotDebug.logSwallowed("DirectCardSubmit POST tới " + siteUrl + ": " + reason, null);
            return new AttemptResult(null, reason);
        } catch (Throwable t) {
            com.naptien.utils.PayBotDebug.logSwallowed("DirectCardSubmit POST tới " + siteUrl + " ném exception", t);
            return new AttemptResult(null, t.getClass().getSimpleName() + ": " + t.getMessage());
        }
    }

    private static AttemptResult tryGetSubmit(String siteUrl, String partnerId, String partnerKey,
                                            String telco, int amount, String cardCode, String cardSerial, String requestId,
                                            String sign, String userAgent) {
        try {
            ...
            String errBody = readErrorBodySafely(conn);
            String reason = "HTTP " + code + (errBody.isEmpty() ? "" : " — " + errBody);
            com.naptien.utils.PayBotDebug.logSwallowed("DirectCardSubmit GET tới " + siteUrl + ": " + reason, null);
            return new AttemptResult(null, reason);
        } catch (Throwable t) {
            com.naptien.utils.PayBotDebug.logSwallowed("DirectCardSubmit GET tới " + siteUrl + " ném exception", t);
            return new AttemptResult(null, t.getClass().getSimpleName() + ": " + t.getMessage());
        }
    }

    /** Đọc error stream an toàn (không ném exception thêm), giới hạn 500 ký tự tránh log quá to. */
