...
public class PayBotConfig {
    ...
    public void load() {
        try {
            ...
            autoMergeAndMigrate();
            // v5.5.5: đồng bộ debug-mode ngay sau khi config sẵn sàng (áp dụng cho cả load lần
            // đầu lẫn /paybot reload, vì cả 2 đường đều gọi qua load() này).
            com.paybot.utils.PayBotDebug.setEnabled(getBoolean("debug-mode", false));
        } catch (Exception e) {
    ...
    public static Map<String, Object> defaultConfig() {
        ...
        cfg.put("vietqr-template",     "qr_only");
        cfg.put("database-type",       "SQLite");
        ...
        transferContent.put("include-date-prefix",    false);
        transferContent.put("check-duplicate-orders", true);
        ...
        // ── Custom Name & Lore ────────────────────────────────────────────────
        Map<String, Object> customName = new LinkedHashMap<>();
        customName.put("bank", new LinkedHashMap<>());
        customName.put("card", new LinkedHashMap<>());
        customName.put("telco", new LinkedHashMap<>());
        cfg.put("custom-name", customName);

        Map<String, Object> customLore = new LinkedHashMap<>();
        customLore.put("enabled", false);
        customLore.put("bank", new LinkedHashMap<>());
        customLore.put("card", new LinkedHashMap<>());
        customLore.put("telco", new LinkedHashMap<>());
        ...
        try (InputStream tplStream = PayBotConfig.class.getResourceAsStream("/config-template.yml")) {
            ...
            for (List<String> block : liveBlocks) {
                ...
                if (key != null && !tplKeys.contains(key)) {
                    ...
                    boolean hasRealData = checkBlockHasRealData(block);
                    boolean isDefaultKey = defaultConfig().containsKey(key);
                    if (hasRealData || isDefaultKey) {
                        PayBotMod.LOGGER.debug("[PayBot] SmartConfigMerger: giữ lại key '" + key
                                + "' (thuộc defaultConfig hoặc có dữ liệu thật — KHÔNG xóa).");
                        keptLines.addAll(block);
                        continue;
                    }
                    PayBotMod.LOGGER.debug("[PayBot] SmartConfigMerger: xoá key thừa '" + key
                            + "' (không còn trong template/defaultConfig, mọi giá trị đều mặc định/rỗng): "
            ...
            for (List<String> block : tplBlocks) {
                ...
                added++;
                PayBotMod.LOGGER.debug("[PayBot] SmartConfigMerge: thêm key mới '" + key
    ...
    private boolean appendMissingBlocksFromTemplate() {
        try (InputStream tplStream = PayBotConfig.class.getResourceAsStream("/config-template.yml")) {
            ...
            List<String> tplLines = new ArrayList<>();
            try (java.io.BufferedReader br = new java.io.BufferedReader(
            ...
            try (java.io.BufferedReader br = new java.io.BufferedReader(
                    new java.io.InputStreamReader(Files.newInputStream(configPath), StandardCharsets.UTF_8))) {
                String line;
                while ((line = br.readLine()) != null) liveLines.add(line);
            }

            List<ConfigBlock> tplBlocks = parseConfigBlocks(tplLines);
            List<ConfigBlock> liveBlocks = parseConfigBlocks(liveLines);

            Map<String, ConfigBlock> tplBlockMap = new HashMap<>();
            for (ConfigBlock b : tplBlocks) if (b.topKey != null) tplBlockMap.put(b.topKey, b);

            Set<String> liveTopKeys = new LinkedHashSet<>();
            for (ConfigBlock b : liveBlocks) if (b.topKey != null) liveTopKeys.add(b.topKey);

            boolean commentsRestored = false;
            List<String> updatedLines = new ArrayList<>();
            for (ConfigBlock b : liveBlocks) {
                if (b.topKey != null && tplBlockMap.containsKey(b.topKey)) {
                    if (restoreBlockComments(b, tplBlockMap.get(b.topKey))) {
                        commentsRestored = true;
                        PayBotMod.LOGGER.info("[PayBot] SmartConfigMerge: khôi phục comment/note bị xoá cho key '{}'.", b.topKey);
                    }
                }
                updatedLines.addAll(b.lines);
            }

            List<String> toAppend = new ArrayList<>();
            int added = 0;
            for (ConfigBlock block : tplBlocks) {
                if (block.topKey == null) continue;
                if (liveTopKeys.contains(block.topKey)) continue;

                if (toAppend.isEmpty() && !updatedLines.isEmpty()) toAppend.add("");
                toAppend.addAll(block.lines);
                toAppend.add("");
                added++;
                PayBotMod.LOGGER.info("[PayBot] SmartConfigMerge: thêm key mới '{}' (kèm comment) vào config.yml.", block.topKey);
            }

            if (added > 0 || commentsRestored) {
                updatedLines.addAll(toAppend);
                try (PrintWriter pw = new PrintWriter(new OutputStreamWriter(Files.newOutputStream(configPath), StandardCharsets.UTF_8))) {
    ...
    private static List<ConfigBlock> parseConfigBlocks(List<String> allLines) {
        ...
        for (String line : allLines) {
            ...
            boolean isTopKey  = !isBlank && !isComment && !isIndented && !line.trim().startsWith("-") && line.contains(":");

            if (isTopKey) {
                current = new ConfigBlock();
                int colon = line.indexOf(':');
                current.topKey = colon > 0 ? line.substring(0, colon).trim() : line.trim();
    ...
    private static boolean restoreBlockComments(ConfigBlock liveBlock, ConfigBlock tplBlock) {
        if (liveBlock == null || tplBlock == null) return false;
        boolean modified = false;

        List<String> tplPreComments = new ArrayList<>();
        for (String line : tplBlock.lines) {
            if (line.trim().startsWith("#")) {
                tplPreComments.add(line);
            } else if (line.contains(":") && !line.startsWith(" ")) {
                break;
            }
        }

        boolean liveHasTopComments = false;
        for (String line : liveBlock.lines) {
            if (line.trim().startsWith("#")) {
                liveHasTopComments = true;
                break;
            } else if (line.contains(":") && !line.startsWith(" ")) {
                break;
            }
        ...
        if (!liveHasTopComments && !tplPreComments.isEmpty()) {
            int topKeyIdx = 0;
            for (int i = 0; i < liveBlock.lines.size(); i++) {
                String line = liveBlock.lines.get(i);
                if (line.contains(":") && !line.startsWith(" ") && !line.trim().startsWith("#")) {
                    topKeyIdx = i;
                    break;
                }
            }
            liveBlock.lines.addAll(topKeyIdx, tplPreComments);
            modified = true;
        }

        for (int i = 0; i < tplBlock.lines.size(); i++) {
            String tplLine = tplBlock.lines.get(i);
            if (tplLine.trim().startsWith("#") && i + 1 < tplBlock.lines.size()) {
                String nextTplLine = tplBlock.lines.get(i + 1);
                if (nextTplLine.startsWith(" ") && nextTplLine.contains(":")) {
                    int c = nextTplLine.indexOf(':');
                    String subKey = c > 0 ? nextTplLine.substring(0, c).trim() : "";
                    if (!subKey.isEmpty()) {
                        for (int j = 0; j < liveBlock.lines.size(); j++) {
                            String liveLine = liveBlock.lines.get(j);
                            if (liveLine.startsWith(" ") && liveLine.contains(":")) {
                                int lc = liveLine.indexOf(':');
                                String liveSubKey = lc > 0 ? liveLine.substring(0, lc).trim() : "";
                                if (subKey.equals(liveSubKey)) {
                                    boolean commentPresent = (j > 0 && liveBlock.lines.get(j - 1).trim().startsWith("#"));
                                    if (!commentPresent) {
                                        liveBlock.lines.add(j, tplLine);
                                        modified = true;
                                    }
                                    break;
                                }
