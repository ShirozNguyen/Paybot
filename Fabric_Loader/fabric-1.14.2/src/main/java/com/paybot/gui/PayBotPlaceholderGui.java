...
import com.paybot.compat.GuiBackend;
import com.paybot.compat.GuiFactory;
import com.paybot.managers.LocalOrderManager;
import com.paybot.managers.PlaceholderManager;
import com.paybot.managers.PlaceholderManager.PlayerStat;
import com.paybot.managers.PlaceholderManager.ServerStat;
...
public class PayBotPlaceholderGui {
    ...
    public static void openMain(ServerPlayer player) {
        ...
        GuiBackend gui = GuiFactory.create(MenuType.GENERIC_9x5, player, "§6§l✦ PayBot — Thống Kê Nạp Tiền ✦");
    ...
    public static void openTopBank(ServerPlayer player) {
        PayBotMod mod = PayBotMod.getInstance();
        List<PlayerStat> top = mod.getPlaceholderManager().getTopByBank(10);
        ServerStat total = mod.getPlaceholderManager().getServerTotal();

        GuiBackend gui = GuiFactory.create(MenuType.GENERIC_9x6, player, "§6§l🏦 Bảng Xếp Hạng Nạp Ngân Hàng");
    ...
    public static void openTopCard(ServerPlayer player) {
        PayBotMod mod = PayBotMod.getInstance();
    ...
    public static void openTopCombined(ServerPlayer player) {
        PayBotMod mod = PayBotMod.getInstance();
        List<PlayerStat> top = mod.getPlaceholderManager().getTopCombined(10);
        ServerStat total = mod.getPlaceholderManager().getServerTotal();

        GuiBackend gui = GuiFactory.create(MenuType.GENERIC_9x6, player, "§a§l🌐 Bảng Xếp Hạng Tổng Hợp");
    ...
    public static void openServerStats(ServerPlayer player) {
        PayBotMod mod = PayBotMod.getInstance();
        PlaceholderManager ph = mod.getPlaceholderManager();
        ServerStat total = ph.getServerTotal();
        ServerStat today = ph.getTodayStat();

        GuiBackend gui = GuiFactory.create(MenuType.GENERIC_9x5, player, "§d§l📊 Thống Kê Toàn Server");
    ...
    public static void openPersonal(ServerPlayer player) {
        PayBotMod mod = PayBotMod.getInstance();
        PlaceholderManager ph = mod.getPlaceholderManager();
        ...
        GuiBackend gui = GuiFactory.create(MenuType.GENERIC_9x5, player, "§e§l👤 Thống Kê Của " + name);
    ...
    public static void openHistory(ServerPlayer player, int page) {
        ...
        GuiBackend gui = GuiFactory.create(MenuType.GENERIC_9x6, player, "§c§l📜 Lịch Sử Giao Dịch §7(Trang " + (page + 1) + "/" + pages + ")");
    ...
    private static void setNavigation(GuiBackend gui, ServerPlayer player) {
    ...
    private static net.minecraft.world.item.Item getRankItem(int rank) {
        return switch (rank) {
