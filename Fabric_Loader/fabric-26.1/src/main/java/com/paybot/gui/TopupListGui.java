...
public class TopupListGui {
    ...
    public static void openBankOnly(ServerPlayer player, int page, String playerFilter) {
        open(player, page, playerFilter, "bank");
    }

    /** /cardcheck [player] — chỉ đơn thẻ cào */
    public static void openCardOnly(ServerPlayer player, int page, String playerFilter) {
        open(player, page, playerFilter, "card");
    }

    // ─── Core: unified paged list ─────────────────────────────────────────────

    private static void open(ServerPlayer player, int page,
        ...
        GuiBackend gui = GuiFactory.create(MenuType.GENERIC_9x6, player, modeLabel + filterSuffix + pageStr);
    ...
    private static void approveBank(PayBotMod mod, LocalOrderManager.BankOrder b,
                                    ServerPlayer admin, int page, String mode, String filter) {
        ...
        try { if (!rawAmt.isEmpty()) rewardAmt = (int)(Double.parseDouble(rawAmt) * mode2); }
            ...
            // v5.5.5 Part 55 [BUG NHỎ — audit]: TRƯỚC ĐÂY im lặng. rewardAmt feed vào placeholder
            // [amount] trong lệnh thưởng — nếu config "denom-rewards-bank.X.amt" gõ sai định
            // dạng, người chơi được admin duyệt tay nhưng có thể nhận thưởng 0 (tuỳ lệnh cấu
            // hình có dùng [amount] hay không). Khác các bug trước (dữ liệu bên ngoài không tin
            // cậy được), đây là lỗi CONFIG (admin tự gõ) nên mức độ thấp hơn — vẫn nên log.
            PayBotMod.LOGGER.warn("[TopupListGui] denom-rewards-bank." + b.amount + ".amt=\"" + rawAmt
                    + "\" không parse được — [amount] sẽ là 0 trong lệnh thưởng.");
        }
