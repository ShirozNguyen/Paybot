package com.naptien.fabric.v1_19_x;
...
/**
 * FabricVersionAdapter1_19 — v5.5.5 Part 44 [ĐÃ HARDEN]
 *
 * Adapter NBT thô cho nhánh Minecraft 1.19.x (module fabric-legacy/, biên dịch
 ...
 */
public class FabricVersionAdapter1_19 implements VersionAdapter {
    private static final Logger LOGGER = LoggerFactory.getLogger("PayBot-Fabric-Adapter1_19_x");
    ...
    private void setName(ItemStack stack, Component nameComponent) {
        try {
            ...
            PayBotDebug.logSwallowed("FabricVersionAdapter1_19.setName: setHoverName() lỗi", t);
        }
    ...
    private void setLore(ItemStack stack, List<String> lore) {
        CompoundTag displayTag = getOrCreateDisplayTag(stack);
        if (displayTag == null) {
            PayBotDebug.logSwallowed("FabricVersionAdapter1_19.setLore: không lấy được display tag — bỏ qua set lore.", null);
            return;
        }
    ...
    public MapItemSavedData getMapSavedData(ItemStack mapItem, ServerLevel world) {
        ...
        try {
            ...
            LOGGER.error("[Fabric1_19_x] getMapSavedData error: {}", t.getMessage());
        }
        return null;
    }
    ...
    public void lockMap(MapItemSavedData state) {
        ...
        if (lockedField != null) {
            try {
                ...
                PayBotDebug.logSwallowed("FabricVersionAdapter1_19.lockMap: set field 'locked' thất bại", t);
            }
        }
        try {
            ...
            PayBotDebug.logSwallowed("FabricVersionAdapter1_19.lockMap: cả field lẫn method lock() đều không dùng được — "
    ...
    private static synchronized void initFieldReflection() {
        ...
        if (boolFields.size() == 1) {
            ...
            PayBotDebug.logSwallowed("FabricVersionAdapter1_19.initFieldReflection: có " + boolFields.size()
    ...
    private CompoundTag getOrCreateTag(ItemStack stack) {
        try {
            ...
            PayBotDebug.logSwallowed("FabricVersionAdapter1_19.getOrCreateTag", t);
            return null;
        }
    }
    ...
    private CompoundTag getTag(ItemStack stack) {
        try {
            ...
            PayBotDebug.logSwallowed("FabricVersionAdapter1_19.getTag", t);
            return null;
        }
    }
    ...
    private CompoundTag getOrCreateDisplayTag(ItemStack stack) {
        ...
        try {
            ...
            PayBotDebug.logSwallowed("FabricVersionAdapter1_19.getOrCreateDisplayTag", t);
            return null;
        }
    }
    ...
    private String safeComponentToJson(Component comp) {
        ...
        try {
            ...
            PayBotDebug.logSwallowed("FabricVersionAdapter1_19.safeComponentToJson", t);
