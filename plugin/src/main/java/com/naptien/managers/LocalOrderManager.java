...
/**
 * LocalOrderManager — v5.1.0 (Refactored to SQLite)
 * <p>
 * Quản lý đơn nạp thẻ / ngân hàng (standalone mode). Dữ liệu persist
 * bằng SQLite thay vì YAML (xem DatabaseManager.java).
 * <p>
 * In-memory cache: mọi đơn được giữ trong RAM để đọc nhanh; mọi thay đổi
 * được ghi thẳng xuống DB ngay lập tức (write-through cache). Đảm bảo:
 * <ul>
 *   <li>Đọc → chỉ cần đọc map trong RAM (O(1))</li>
 *   <li>Ghi → ghi RAM + SQL trong cùng 1 lời gọi (atomic từ góc nhìn caller)</li>
 *   <li>Restart → load lại từ DB, khôi phục đầy đủ trạng thái</li>
 * </ul>
 * <p>
 * Changelog:
 *   v4.1.0 — Tái tạo từ .class + tích hợp LogManager
 *   v4.2.0 — Dual-write YAML (main + mirror Bank/Card)
 *   v5.0.0 — matchPendingByContent + pruneOldOrders
 *   v5.1.0 — Migrate hoàn toàn sang SQLite, bỏ YAML
 */
public class LocalOrderManager {
    ...
    public LocalOrderManager(NapTienPlugin plugin) {
        this.plugin = plugin;
    ...
    private void loadBankOrders() {
        ...
        plugin.getLogger().info("[PayBot] Loaded " + bankOrders.size() + " bank order(s) từ database.");
    }
    ...
    private void loadCardOrders() {
        ...
        plugin.getLogger().info("[PayBot] Loaded " + cardOrders.size() + " card order(s) từ database.");
    }
    ...
    public synchronized void createBankOrder(String invoiceId, String playerName, int amount) {
        ...
        plugin.getLogManager().logBank(playerName, amount, invoiceId,
    ...
    public synchronized void updateBankStatus(String invoiceId, String newStatus) {
        ...
        plugin.getLogManager().logBank(o.playerName, o.amount, invoiceId,
    ...
    /**
     * Tìm BankOrder đang chờ khớp với nội dung chuyển khoản.
     * Khớp nếu mã đơn (invoiceId, viết hoa) xuất hiện trong nội dung CK,
     * hoặc đúng bằng field "code".
     */
    ...
    public synchronized void createCardOrder(String requestId, String playerName,
        ...
        plugin.getLogManager().logCard(playerName, denom, cardCode, cardSerial,
    ...
    public synchronized void updateCardStatus(String requestId, String newStatus, String message) {
        ...
        if (!CARD_PROCESSING.equals(newStatus)) {
            plugin.getLogManager().logCard(o.playerName, o.denom, o.cardCode, o.cardSerial,
    ...
    // ─── File paths (hiển thị trong /bankcheck, /cardcheck) ──────────────────

    public String getBankDataFilePath() {
        return new java.io.File(plugin.getDataFolder(), "paybot.db").getAbsolutePath();
    }
    ...
    /**
     * Xoá các đơn ở trạng thái CUỐI đã quá hạn lưu trữ.
     * Đơn PENDING/PROCESSING KHÔNG BAO GIỜ bị xoá.
     *
     * @param bankRetentionDays -1 = giữ vĩnh viễn; 0 = xoá ngay; >0 = xoá sau N ngày
     * @param cardRetentionDays cùng quy ước
     * @return số đơn đã xoá
     */
    public synchronized int pruneOldOrders(int bankRetentionDays, int cardRetentionDays) {
        ...
        if (bankRetentionDays >= 0) {
            ...
            db.deleteBankOrdersBefore(cutoff, terminalStatuses);
