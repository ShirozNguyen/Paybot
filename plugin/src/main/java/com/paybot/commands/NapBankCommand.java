...
import com.paybot.gui.NapBankGui;
...
public final class NapBankCommand implements CommandExecutor {
    ...
    public NapBankCommand(PayBotPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        try {
            ...
            player.sendMessage(PayBotPlugin.f("§c[PayBot] §fSố tiền không hợp lệ! VD: §e/napbank 50000 §fhoặc §e/napbank 50k"));
            return true;
        }
    ...
    public void requestAndGiveQR(Player player, int amount) {
        if (amount < 10_000) {
            player.sendMessage(PayBotPlugin.f("§c[PayBot] §fSố tiền tối thiểu là §e10,000 VND§f."));
            return;
        }
        if (amount > 50_000_000) {
            player.sendMessage(PayBotPlugin.f("§c[PayBot] §fSố tiền quá lớn! Vui lòng liên hệ admin."));
            return;
        }
        if (!isSepayConfigured()) {
            player.sendMessage(PayBotPlugin.f("§c[PayBot] §fServer chưa được cấu hình SePay."));
        ...
        SchedulerUtils.runAsync(plugin, () -> {
            String invoiceId   = com.paybot.managers.TransferContentGenerator.generate(plugin);
            String bankAccount = plugin.getConfig().getString("sepay-api.bank-account-number", "").trim();
            if (bankAccount.isEmpty()) bankAccount = plugin.getConfig().getString("sepay.bank-account", "").trim();

            String bankName = plugin.getConfig().getString("sepay-api.bank-short-name", "").trim();
            if (bankName.isEmpty()) bankName = plugin.getConfig().getString("sepay.bank-name", "").trim();

            String accountName = plugin.getConfig().getString("sepay-api.account-holder-name", "").trim();
            if (accountName.isEmpty()) accountName = plugin.getConfig().getString("sepay.account-name", "").trim();

            if (bankAccount.isEmpty() || bankName.isEmpty()) {
                SchedulerUtils.runForPlayer(plugin, player, () ->
                    player.sendMessage(PayBotPlugin.f("§c[PayBot] §fChưa cấu hình SePay! Admin dùng §e/sepaysetup§f.")));
                return;
            }
            ...
            String qrPrimary  = buildVietQRUrl(plugin, bankName, bankAccount, amount, invoiceId);
            ...
            final String finalBankAccount = bankAccount;
            final String finalBankName = bankName;
            final String finalAccountName = accountName;

            SchedulerUtils.runForPlayer(plugin, player, () -> {
                player.sendMessage(PayBotPlugin.f("§e[PayBot] §fĐang tạo QR chuyển khoản..."));
                if (showOverlay) {
                    plugin.getQRMapManager().giveQRMap(player, qrPrimary, qrFallback, invoiceId,
                            finalBankName, finalBankAccount, finalAccountName, amount);
                } else {
                    plugin.getQRMapManager().giveQRMap(player, qrPrimary, qrFallback, invoiceId);
                }
     ...
     */
    private static String buildVietQRUrl(PayBotPlugin plugin, String bankName, String bankAccount, int amount, String invoiceId) {
        try {
            String bin      = resolveBankBin(bankName);
            String bankId   = (bin != null) ? bin : bankName.trim();
            String template = plugin.getConfig().getString("vietqr-template", "qr_only").trim();
            if (template.isEmpty()) template = "qr_only";
            return "https://img.vietqr.io/image/"
                    + java.net.URLEncoder.encode(bankId,      "UTF-8") + "-"
                    + java.net.URLEncoder.encode(bankAccount, "UTF-8")
                    + "-" + java.net.URLEncoder.encode(template, "UTF-8") + ".png"
                    + "?amount=" + amount
