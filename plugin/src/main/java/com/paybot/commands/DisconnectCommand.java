package com.paybot.commands;

import com.paybot.PayBotPlugin;
import com.paybot.utils.SchedulerUtils;
import org.bukkit.command.*;
import org.bukkit.entity.Player;

public class DisconnectCommand implements CommandExecutor {

    private final PayBotPlugin plugin;

    public DisconnectCommand(PayBotPlugin plugin) { this.plugin = plugin; }

    @Override
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (!sender.hasPermission("naptien.admin")) {
            sender.sendMessage("§cYou Don't Have Permission To Use This!");
            return true;
        }
        // [DEAD CODE — Bot-connected mode đã tắt] Xem comment cùng loại trong
        // ConnectCommand.java — giữ nguyên code bên dưới, chỉ chặn từ đây.
        if (plugin.isStandaloneMode()) {
            PayBotPlugin.sendBotDisabledNotice(sender);
            return true;
        }
        // BUG FIX v4.1.0: Chỉ check guild-id (không phải server-id || guildId).
        // server-id luôn có giá trị (tự sinh) nên điều kiện cũ (serverId && guildId cùng rỗng)
        // không bao giờ đúng → luôn cố gọi bot dù chưa kết nối.
        String guildId = plugin.getConfig().getString("guild-id", "").trim();
        if (guildId.isEmpty()) {
            sender.sendMessage(PayBotPlugin.f("§c[PayBot] §fPlugin chưa kết nối hệ thống auto."));
            return true;
        }

        // --force: xoá config cục bộ ngay mà không cần liên hệ bot
        boolean force = args.length > 0 && args[0].equalsIgnoreCase("--force");
        if (force) {
            clearBotConfig();
            sender.sendMessage(PayBotPlugin.f("§a[PayBot] §fĐã ngắt kết nối cục bộ (force)."));
            sender.sendMessage("§7Guild §e" + guildId + " §7đã bị xoá khỏi config.");
            return true;
        }
        sender.sendMessage(PayBotPlugin.f("§e[PayBot] §fĐang gửi yêu cầu ngắt kết nối..."));
        // sender có thể là console (không có region) — chỉ Player mới cần entity scheduler.
        Player senderPlayer = (sender instanceof Player p) ? p : null;
        SchedulerUtils.runAsync(plugin, () -> {
            boolean ok = plugin.getBotHttpClient().requestDisconnect();
            Runnable syncBack = () -> {
                if (ok) {
                    sender.sendMessage(PayBotPlugin.f("§a[PayBot] §fYêu cầu đã gửi thành công!"));
                    sender.sendMessage("§7Dùng §e/confirm §7để xác nhận ngắt kết nối.");
                } else {
                    sender.sendMessage(PayBotPlugin.f("§c[PayBot] §fGửi yêu cầu thất bại!"));
                    sender.sendMessage("§7Nếu bot không phản hồi, dùng §e/disconnect --force §7để xoá config cục bộ.");
                }
            };
            if (senderPlayer != null) SchedulerUtils.runForPlayer(plugin, senderPlayer, syncBack);
            else SchedulerUtils.runSync(plugin, syncBack);
        });
        return true;
    }

    private void clearBotConfig() {
        plugin.getConfig().set("guild-id",            "");
        plugin.getConfig().set("reward-command",      "");
        plugin.getConfig().set("reward-command-card", "");
        plugin.getConfig().set("reward-command-bank", "");
        plugin.getConfig().set("denom-rewards",       null);
        plugin.getConfig().set("denom-rewards-card",  null);
        plugin.getConfig().set("denom-rewards-bank",  null);
        plugin.saveConfig();
    }
}
