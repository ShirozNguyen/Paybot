...
import com.paybot.managers.TransferContentGenerator;
import com.paybot.managers.SmartConfigMerger;
...
public final class ReloadCommand implements CommandExecutor {
    ...
    public ReloadCommand(final PayBotPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        if (!sender.hasPermission("naptien.admin")) {
            sender.sendMessage(PayBotPlugin.f("§c[PayBot] §fBạn không có quyền thực hiện lệnh này (cần §enaptien.admin§f)."));
            return true;
        }

        sender.sendMessage(PayBotPlugin.f("§e[PayBot] §fĐang tiến hành nạp lại và áp dụng (apply) cấu hình..."));
        ...
        try {
            ...
            sender.sendMessage(PayBotPlugin.f("§a[PayBot] §f✓ Đã nạp lại và áp dụng §a(apply) §ftoàn bộ cấu hình mới từ config.yml thành công!"));
            plugin.getLogger().info("[PayBot] Admin " + sender.getName() + " đã thực hiện /paybot reload thành công.");
        } catch (final Exception e) {
            sender.sendMessage(PayBotPlugin.f("§c[PayBot] §fLỗi khi reload cấu hình: " + e.getMessage()));
