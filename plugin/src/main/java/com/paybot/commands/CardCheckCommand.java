...
import com.paybot.gui.TopupListGui;
...
public class CardCheckCommand implements CommandExecutor {
    ...
    public CardCheckCommand(PayBotPlugin plugin, BankCheckCommand bankCheck) {
        this.plugin    = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (plugin.isStandaloneMode()) {
            ...
            sender.sendMessage(PayBotPlugin.f("§c[PayBot] §fHệ thống tự động đang hoạt động, đơn được xử lý tự động."));
        }
        return true;
    }
    ...
    private void handleStandalone(CommandSender sender, String[] args) {
        ...
        if (count % 15 == 0) {
            sender.sendMessage(PayBotPlugin.f("§6[PayBot] §fNếu quá mệt với việc làm thủ công bạn có thể cân nhắc inbox admin để setup hệ thống auto giao dịch nhé :3"));
        }
        ...
        if (args.length == 0) {
            ...
            if (all.isEmpty()) {
                sender.sendMessage(PayBotPlugin.f("§7[PayBot] Chưa có đơn nạp thẻ nào."));
