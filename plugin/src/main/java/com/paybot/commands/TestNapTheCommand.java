...
import com.paybot.gui.GuiSession;
...
public final class TestNapTheCommand implements CommandExecutor {
    ...
    public TestNapTheCommand(PayBotPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!player.isOp()) {
            player.sendMessage(PayBotPlugin.f("§c[PayBot] §fLệnh test chỉ dành cho OP!"));
            return true;
        }
        ...
        if (args.length >= 2) {
            ...
            int    amount = com.paybot.gui.GuiUtil.parseDenom(args[1].toLowerCase());
            if (amount <= 0) {
                player.sendMessage(PayBotPlugin.f("§c[PayBot] §fMệnh giá không hợp lệ! Vd: §e/testnapthe VIETTEL 100k"));
                return true;
            }
        ...
        s.testMode = true;
        player.sendMessage(PayBotPlugin.f("§e§l[TEST MODE] §r§eChọn nhà mạng rồi mệnh giá — thẻ sẽ được "
