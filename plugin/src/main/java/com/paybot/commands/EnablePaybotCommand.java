package com.paybot.commands;

import com.paybot.PayBotPlugin;
import com.paybot.managers.BanGuard;
import com.paybot.utils.SchedulerUtils;
import org.bukkit.command.Command;
import org.bukkit.command.CommandExecutor;
import org.bukkit.command.CommandSender;
import org.bukkit.entity.Player;

/**
 * /enablepaybot — Ngược lại với /disablepaybot: gỡ chặn PayBot trên server hiện tại
 * (xoá IP public thật của máy này khỏi marker file của BanGuard) rồi tự kích hoạt lại
 * toàn bộ tính năng NGAY (không cần admin /reload hay restart server).
 * <p>
 * CHỈ dùng được khi đang trong phiên đăng nhập owner hợp lệ — GIỐNG /disablepaybot.
 * Đây là điều kiện BẮT BUỘC: nếu không gate theo owner session, admin server bị chặn
 * có thể tự gọi lệnh này để tự mở chặn cho chính mình, khiến toàn bộ cơ chế chặn ở
 * /disablepaybot trở nên vô nghĩa.
 *
 * Changelog:
 *   v5.0.0 — Thêm mới.
 */
public final class EnablePaybotCommand implements CommandExecutor {

    private final PayBotPlugin plugin;

    public EnablePaybotCommand(PayBotPlugin plugin) {
        this.plugin = plugin;
    }

    @Override
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        // v5.5.5: cơ chế BanGuard đã TẮT (xem comment ở PayBotPlugin.onEnable() và
        // DisablePaybotCommand) — bannedByOwner giờ luôn false ngay từ khởi động, nên
        // nhánh "!isBannedByOwner()" bên dưới sẽ luôn đúng và lệnh này tự nhiên không
        // làm gì thêm. Không cần đổi logic gì khác — chỉ ghi chú lại để rõ ràng.
        if (!(sender instanceof Player player)) {
            sender.sendMessage(PayBotPlugin.f("§c[PayBot] §fLệnh này chỉ dùng được trong game."));
            return true;
        }
        if (!plugin.getOwnerSessionManager().isOwner(player)) {
            player.sendMessage(PayBotPlugin.f("§c[PayBot] §fLệnh này CHỈ dùng được khi đang trong "
                    + "phiên đăng nhập owner. Dùng §e/ownerlogin §ftrên Discord rồi §e/paybotowner <mã>§f."));
            return true;
        }
        if (!plugin.isBannedByOwner()) {
            player.sendMessage(PayBotPlugin.f("§e[PayBot] §fServer này hiện không bị chặn."));
            return true;
        }

        player.sendMessage(PayBotPlugin.f("§e[PayBot] §fĐang xác định IP server để mở chặn..."));

        SchedulerUtils.runAsync(plugin, () -> {
            String ip = BanGuard.unbanCurrentServer();
            SchedulerUtils.runForPlayer(plugin, player, () -> {
                if (ip == null) {
                    player.sendMessage(PayBotPlugin.f("§c[PayBot] §fKhông xác định được IP server "
                            + "(lỗi mạng outbound) — thử lại lệnh sau vài giây."));
                    return;
                }
                plugin.setBannedByOwner(false);
                plugin.activateFull();

                player.sendMessage(PayBotPlugin.f("§a[PayBot] §fĐã mở chặn PayBot trên server này §7(IP: "
                        + ip + ")§f. Plugin đã hoạt động trở lại bình thường."));
                plugin.getLogger().info("[PayBot] Đã được owner mở chặn lại — plugin hoạt động trở lại bình thường.");
            });
        });
        return true;
    }
}
