...
import com.paybot.managers.NotificationManager;
...
public class ApproveCommand implements CommandExecutor {
    ...
    public ApproveCommand(PayBotPlugin plugin) { this.plugin = plugin; }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!plugin.isStandaloneMode()) {
            sender.sendMessage(PayBotPlugin.f("§c[PayBot] §fHệ thống tự động đang hoạt động. Lệnh này chỉ dùng khi chưa kết nối hệ thống auto."));
            return true;
        }
        if (args.length == 0) {
            sender.sendMessage(PayBotPlugin.f("§c[PayBot] §fCách dùng: §e/approve <invoice_id hoặc request_id>"));
            return true;
        }
        ...
        if (bank != null) {
            try {
                ...
                sender.sendMessage(PayBotPlugin.f("§c[PayBot] §fDuyệt đơn lỗi! Xem console để biết thêm."));
            }
            return true;
        }
        ...
        sender.sendMessage(PayBotPlugin.f("§c[PayBot] §fKhông tìm thấy đơn với ID: §e" + idArg));
    ...
    private void approveBankOrder(CommandSender sender, LocalOrderManager.BankOrder order, LocalOrderManager lom) {
        if (LocalOrderManager.BANK_APPROVED.equals(order.status)) {
            sender.sendMessage(PayBotPlugin.f("§c[PayBot] §fĐơn §e" + order.invoiceId + " §fđã được approve trước đó."));
            return;
        }
        if (!LocalOrderManager.BANK_PAID.equals(order.status)) {
            sender.sendMessage(PayBotPlugin.f("§c[PayBot] §fĐơn §e" + order.invoiceId
        ...
        if (rewardCmds.isEmpty()) {
            sender.sendMessage(PayBotPlugin.f("§c[PayBot] §fChưa cấu hình lệnh thưởng cho mệnh giá §e"
        ...
        sender.sendMessage(PayBotPlugin.f("§a[PayBot] §fĐã approve đơn §b#" + order.invoiceId
    ...
    private void approveCardOrder(CommandSender sender, LocalOrderManager.CardOrder order, LocalOrderManager lom) {
        if (LocalOrderManager.CARD_APPROVED.equals(order.status)) {
            sender.sendMessage(PayBotPlugin.f("§c[PayBot] §fĐơn §e" + order.requestId + " §fđã được approve trước đó."));
            return;
        }
        if (!LocalOrderManager.CARD_SUCCESS.equals(order.status)) {
            sender.sendMessage(PayBotPlugin.f("§c[PayBot] §fChỉ approve được đơn thành công (trạng thái hiện tại: §7"
                    + order.status + "§f)."));
            return;
        }
        ...
        sender.sendMessage(PayBotPlugin.f("§a[PayBot] §fĐã approve đơn thẻ §b#" + order.requestId.substring(0, 8)
