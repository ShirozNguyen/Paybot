 ...
 * Giờ chỉ báo 1 dòng ngắn lúc join (xem PayBotPlugin#onPlayerJoin), player TỰ chủ động
...
public final class RewardClaimCommand implements CommandExecutor {
    ...
    public RewardClaimCommand(PayBotPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (reward == null) {
            player.sendMessage(PayBotPlugin.f("§7[PayBot] §fBạn không có phần thưởng nào đang chờ nhận."));
            return true;
        }
        ...
        if (args.length > 0 && args[0].equalsIgnoreCase("confirm")) {
            ...
            if (remaining > 0) {
                player.sendMessage(PayBotPlugin.f("§7[PayBot] §fBạn còn §a" + remaining
        ...
        if ("bank".equalsIgnoreCase(type)) {
            ...
            player.sendMessage(PayBotPlugin.f("§6§l▬▬▬▬▬▬▬ §eXÁC NHẬN NHẬN THƯỞNG §6▬▬▬▬▬▬▬"));
        ...
        player.sendMessage(PayBotPlugin.f("§a✔ §fGõ §a/RewardClaim confirm §fđể nhận thưởng ngay."));
        return true;
    }
