...
import com.paybot.gui.NapTheGui;
...
public final class NapTheCommand implements CommandExecutor {
    ...
    public NapTheCommand(PayBotPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!isReady()) {
            player.sendMessage(PayBotPlugin.f("§c[PayBot] §fServer chưa được cấu hình API gạch thẻ."));
        ...
        if (args.length < 4) {
            player.sendMessage(PayBotPlugin.f("§c[PayBot] §fCách dùng: §e/napthe §7[nhà mạng] [mệnh giá] [mã thẻ] [serial]"));
        ...
        if (!isValidTelco(telco)) {
            player.sendMessage(PayBotPlugin.f("§c[PayBot] §fNhà mạng không hợp lệ! Hợp lệ: VIETTEL, VINAPHONE, MOBIFONE, ZING, GATE, GARENA, VCOIN, APPOTA"));
            return true;
        }
        int denomInt = com.paybot.gui.GuiUtil.parseDenom(denom);
        if (denomInt < 0) {
            player.sendMessage(PayBotPlugin.f("§c[PayBot] §fMệnh giá không hợp lệ! Hợp lệ: 10k, 20k, 30k, 50k, 100k, 200k, 300k, 500k, 1000k"));
            return true;
        }
        ...
        player.sendMessage(PayBotPlugin.f("§e[PayBot] §fKiểm tra lại thẻ:"));
        player.sendMessage("§7Nhà mạng: §f" + telco + "  §7Mệnh giá: §f" + com.paybot.gui.GuiUtil.formatVnd(denomInt) + " VND");
