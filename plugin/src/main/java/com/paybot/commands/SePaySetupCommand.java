...
import com.paybot.managers.SePayApiClient;
import com.paybot.managers.SetupManager;
...
public class SePaySetupCommand implements CommandExecutor {
    ...
    public SePaySetupCommand(PayBotPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (plugin.getSetupManager().isInSession(player)) {
            player.sendMessage(PayBotPlugin.f("§c[PayBot] §fBạn đang trong phiên setup khác, gõ §ecancel §ftrong chat để huỷ."));
            return true;
        }
    ...
    private class SePaySession implements SetupManager.SetupSession {
        ...
        public boolean handleInput(Player player, String input) {
            if (busy) {
                player.sendMessage(PayBotPlugin.f("§7[PayBot] §fĐang xử lý, vui lòng đợi..."));
                return false;
            }
            if (input.equalsIgnoreCase("cancel")) {
                player.sendMessage(PayBotPlugin.f("§c[PayBot] §fĐã huỷ setup SePay."));
                return true;
            }
            switch (stage) {
        ...
        private boolean handleAskTutorial(Player player, String input) {
            ...
            player.sendMessage(PayBotPlugin.f("§c[PayBot] §fGõ §ayes §fhoặc §bskip §f(hoặc §ecancel §fđể huỷ):"));
            return false;
        }
        ...
        private boolean handleTutorial(Player player, String input) {
            ...
            player.sendMessage(PayBotPlugin.f("§c[PayBot] §fGõ §fnext §fđể xem tiếp (hoặc §ecancel §fđể huỷ):"));
            return false;
        }
        ...
        private boolean handleAskToken(Player player, String input) {
            if (input.isEmpty()) {
                player.sendMessage(PayBotPlugin.f("§c[PayBot] §fToken không được để trống! Nhập lại:"));
                return false;
            }
            String token = input;
            ...
            busy = true;
            player.sendMessage(PayBotPlugin.f("§e[PayBot] §fĐang kiểm tra token với SePay..."));
            SchedulerUtils.runAsync(plugin, () -> {
                String tokenError = plugin.getSePayApiClient().testToken();
                SchedulerUtils.runForPlayer(plugin, player, () -> {
                    busy = false;
                    if (!tokenError.isEmpty()) {
                        player.sendMessage(PayBotPlugin.f("§c[PayBot] §fToken lỗi: §7" + tokenError));
                        player.sendMessage("§7Dán lại token khác, hoặc gõ §ecancel §7để huỷ:");
                        return;
                    }
                    player.sendMessage(PayBotPlugin.f("§a✓ §fToken hợp lệ! Đang lấy thông tin tài khoản ngân hàng..."));
            ...
            SchedulerUtils.runAsync(plugin, () -> {
                ...
                SchedulerUtils.runForPlayer(plugin, player, () -> {
                    ...
                    if (accounts.isEmpty()) {
                        ...
                        player.sendMessage(PayBotPlugin.f("§c[PayBot] §fServer CHƯA liên kết tài khoản ngân hàng nào trên SePay!"));
                    ...
                    if (accounts.size() == 1) {
                        ...
                        player.sendMessage(PayBotPlugin.f("§e[PayBot] §fTìm thấy §a" + accounts.size() + " §ftài khoản ngân hàng — chọn 1 (gõ số):"));
        ...
        private boolean handleBankNotLinkedConfirm(Player player, String input) {
            if (input.equalsIgnoreCase("confirm")) {
                player.sendMessage(PayBotPlugin.f("§e[PayBot] §fĐang kiểm tra lại..."));
                fetchBankAccounts(player);
                return false;
            }
            player.sendMessage(PayBotPlugin.f("§c[PayBot] §fGõ §aconfirm §fsau khi đã liên kết (hoặc §ccancel §fđể huỷ):"));
            return false;
        }
        ...
        private boolean handlePickBankAccount(Player player, String input) {
            try {
                ...
                if (idx < 0 || idx >= foundAccounts.size()) {
                    player.sendMessage(PayBotPlugin.f("§c[PayBot] §fSố không hợp lệ! Nhập lại:"));
                    return false;
                }
                chosenAccount = foundAccounts.get(idx);
                showFinalSummary(player);
            } catch (NumberFormatException e) {
                player.sendMessage(PayBotPlugin.f("§c[PayBot] §fVui lòng nhập SỐ thứ tự! Nhập lại:"));
            }
            return false;
        }
        ...
        private boolean handleFinalConfirm(Player player, String input) {
            if (!input.equalsIgnoreCase("yes")) {
                player.sendMessage(PayBotPlugin.f("§c[PayBot] §fGõ §ayes §fđể lưu, hoặc §ccancel §fđể huỷ:"));
                return false;
            }
            ...
            player.sendMessage(PayBotPlugin.f("§a§l[PayBot] §r§a✓ Cấu hình SePay hoàn tất!"));
