...
public final class TestNapBankCommand implements CommandExecutor {
    ...
    public TestNapBankCommand(PayBotPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length > 0) {
            int amount = com.paybot.gui.GuiUtil.parseDenom(args[0].toLowerCase());
            if (amount <= 0) {
                player.sendMessage(PayBotPlugin.f("§c[PayBot] §fMệnh giá không hợp lệ! Vd: §e/testnapbank 100k"));
                return true;
            }
        ...
        s.testMode = true;
        player.sendMessage(PayBotPlugin.f("§e§l[TEST MODE] §r§eChọn 1 mệnh giá — đơn sẽ được giả lập "
