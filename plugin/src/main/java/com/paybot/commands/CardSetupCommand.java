...
import com.paybot.managers.StandaloneCardProcessor;
import com.paybot.utils.SchedulerUtils;
import com.paybot.utils.TextHelper;
...
public class CardSetupCommand implements CommandExecutor {
    ...
    public CardSetupCommand(PayBotPlugin plugin) { this.plugin = plugin; }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (plugin.getSetupManager().isInSession(player)) {
            player.sendMessage(PayBotPlugin.f("§c[PayBot] §fBạn đang trong phiên setup khác, gõ §e/cancelpaysetup §fđể huỷ."));
            return true;
        }
    ...
    private class CardSession implements SetupManager.SetupSession {
        ...
        public boolean handleInput(Player player, String input) {
            if (input.isEmpty()) {
                player.sendMessage(PayBotPlugin.f("§c[PayBot] §fGiá trị không được để trống! Nhập lại:"));
                return false;
            }
            switch (step) {
                case SITE: {
                    ...
                    if (!StandaloneCardProcessor.getSupportedSites().containsKey(normalized)) {
                        player.sendMessage(PayBotPlugin.f("§c[PayBot] §fTrang không hợp lệ! Hợp lệ: "
                    ...
                    String[] creds = com.paybot.managers.DirectCardSubmitHandler.resolveCredentials(plugin, normalized);
                    if (!creds[0].isEmpty() && !creds[1].isEmpty()) {
                        plugin.getConfig().set("card-api.partner-id", creds[0]);
                        plugin.getConfig().set("card-api.partner-key", creds[1]);
                        plugin.getConfig().set("card-api.configured", true);
                        plugin.forceConfigRefresh("cấu hình web đổi thẻ tự động từ card-api-sites");
                        player.sendMessage("§a✓ Đã tự động nhận diện Partner ID & Key từ danh sách lưu sẵn cho " + normalized + "!");
