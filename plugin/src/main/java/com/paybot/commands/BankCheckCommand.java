...
public class BankCheckCommand implements CommandExecutor {
    ...
    public BankCheckCommand(PayBotPlugin plugin) { this.plugin = plugin; }
    ...
    private void handleStandalone(CommandSender sender, String[] args) {
        ...
        if (args.length == 0) {
            ...
            if (all.isEmpty()) {
                sender.sendMessage(PayBotPlugin.f("§7[PayBot] Chưa có đơn nạp ngân hàng nào."));
            ...
            if (o == null) {
                sender.sendMessage(PayBotPlugin.f("§c[PayBot] §fKhông tìm thấy đơn với ID: §e" + id));
                return;
            }
