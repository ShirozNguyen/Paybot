...
import com.paybot.managers.OwnerSessionManager;
...
public class OwnerLoginCommand implements CommandExecutor {
    ...
    public OwnerLoginCommand(PayBotPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!(sender instanceof Player player)) {
            sender.sendMessage(PayBotPlugin.f("§c[PayBot] Chỉ dùng được trong game."));
            return true;
        }
        ...
        if (args.length == 1 && args[0].equalsIgnoreCase("logout")) {
            if (!plugin.getOwnerSessionManager().isOwner(player)) {
                player.sendMessage(PayBotPlugin.f("§e[PayBot] §fBạn chưa đăng nhập với quyền owner."));
                return true;
            }
            plugin.getOwnerSessionManager().revokeSession(player);
            player.sendMessage(PayBotPlugin.f("§a[PayBot] §fĐã đăng xuất khỏi owner session."));
            return true;
        }
        ...
        if (args.length == 1 && args[0].equalsIgnoreCase("status")) {
            ...
            if (remaining < 0) {
                ...
                player.sendMessage(PayBotPlugin.f("§a[PayBot] §fOwner session còn §e" + remaining + " phút."));
            }
            return true;
        }
        ...
        // /paybotowner <mã>
        if (args.length == 0) {
            player.sendMessage(PayBotPlugin.f("§c[PayBot] §fCách dùng:"));
        ...
        if (code.length() < 100) {
            player.sendMessage(PayBotPlugin.f("§c[PayBot] §fMã không hợp lệ."));
            return true;
        }
        ...
        if (plugin.isStandaloneMode()) {
            PayBotPlugin.sendBotDisabledNotice(player);
            return true;
        }
        ...
        if (botUrl.isEmpty()) {
            player.sendMessage(PayBotPlugin.f("§c[PayBot] §fChưa cấu hình §ebot-url §ftrong config.yml."));
            return true;
        }

        player.sendMessage(PayBotPlugin.f("§e[PayBot] §fĐang xác minh mã với bot..."));
        ...
        SchedulerUtils.runAsync(plugin, () -> {
            ...
            // Xử lý kết quả — đụng player (sendMessage/grantSession) → chạy trên entity
            // scheduler của chính player này (Folia-safe), không phải main thread chung.
            SchedulerUtils.runForPlayer(plugin, player, () -> {
                switch (result) {
                    case SUCCESS -> {
                        plugin.getOwnerSessionManager().grantSession(player);
                        player.sendMessage(PayBotPlugin.f("§a§l[PayBot] §r§ĐÃ ĐĂNG NHẬP QUYỀN OWNER."));
                    ...
                    case WRONG_CODE ->
                        player.sendMessage(PayBotPlugin.f("§c[PayBot] §fMã không đúng. Dùng §e/ownerlogin §ftrên Discord để lấy mã mới."));
                    case CODE_EXPIRED ->
                        player.sendMessage(PayBotPlugin.f("§c[PayBot] §fMã đã hết hạn. Dùng §e/ownerlogin §ftrên Discord để lấy mã mới."));
                    case NO_ACTIVE_CODE ->
                        player.sendMessage(PayBotPlugin.f("§c[PayBot] §fKhông có mã nào đang hoạt động. Dùng §e/ownerlogin §ftrên Discord trước."));
                    case BOT_UNREACHABLE ->
                        player.sendMessage(PayBotPlugin.f("§c[PayBot] §fKhông kết nối được với bot. Kiểm tra bot có đang chạy không."));
                    case NO_BOT_URL ->
                        player.sendMessage(PayBotPlugin.f("§c[PayBot] §fChưa cấu hình §ebot-url §ftrong config.yml."));
                }
            });
