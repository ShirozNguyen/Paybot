...
public final class TopupListCommand implements CommandExecutor {
    ...
    public TopupListCommand(PayBotPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!plugin.isStandaloneMode()) {
            player.sendMessage(PayBotPlugin.f("§e[PayBot] §fBạn đang kết nối với Discord bot."));
        ...
        player.sendMessage(PayBotPlugin.f("§c[PayBot] §fCách dùng: §e/topuplist all"));
        return true;
    }
}
