...
import com.paybot.managers.RewardDispatcher;
...
public class PluginHttpServer extends NanoHTTPD {
    ...
    public PluginHttpServer(PayBotPlugin plugin) {
    ...
    public Response serve(IHTTPSession session) {
        ...
        try {
            ...
            // PayBotPlugin.isStandaloneMode()). Chặn NGAY TỪ ĐÂY, TRƯỚC CẢ bước kiểm tra
    ...
    private Response handleSepayIpn(IHTTPSession session, Method method) throws Exception {
        ...
        // ── [SECURITY FIX v5.5.5] ────────────────────────────────────────────
        // TRƯỚC ĐÂY: header "x-forwarded-by-paybot: true" (do client tự set, KHÔNG
        // được ký/xác thực bằng bất kỳ cơ chế mật mã học nào) cho phép bypass HOÀN
        // TOÀN việc kiểm tra sepay.secret-key. Bất kỳ ai gửi request kèm header này
        // tới plugin-port đều bypass được xác thực → có thể giả webhook SePay để
        // nhận thưởng mà không cần thanh toán thật. Đã xoá bỏ hoàn toàn cơ chế bypass
        // này — KHÔNG có header nào được phép qua mặt xác thực secret-key.
        //
        // Đồng thời: nếu admin CHƯA cấu hình sepay.secret-key (rỗng), endpoint sẽ
        // TỪ CHỐI thay vì cho qua im lặng như trước — theo đúng khuyến nghị chính
        // thức của SePay (never accept unauthenticated webhooks in production).
        String configuredKey = plugin.getConfig().getString("sepay.secret-key", "").trim();

        if (configuredKey.isEmpty()) {
            NotificationManager.warn(plugin, "sepay-error",
                    "[PayBot] SePay IPN bị từ chối: 'sepay.secret-key' chưa được cấu hình trong config.yml. " +
                    "Vui lòng đặt 1 khoá bí mật mạnh (trùng khớp với API Key đã cấu hình trên dashboard SePay) " +
                    "để bảo vệ endpoint này, nếu không endpoint /api/sepay-ipn sẽ luôn từ chối request.");
            PayBotDebug.logSwallowed(plugin, "SePay IPN: secret-key rỗng, từ chối request từ " + session.getRemoteIpAddress(), null);
            return jsonResponse(Response.Status.UNAUTHORIZED, "{\"success\":false,\"error\":\"sepay.secret-key chưa được cấu hình\"}");
        }
        ...
        if (!receivedKey.equals(configuredKey)) {
            NotificationManager.warn(plugin, "sepay-error",
                    "[PayBot] SePay IPN: auth failed từ IP " + session.getRemoteIpAddress() + ".");
            PayBotDebug.logSwallowed(plugin, "SePay IPN auth failed từ " + session.getRemoteIpAddress()
                    + " (key nhận được: " + (receivedKey.isEmpty() ? "<rỗng>" : "***" + safeSuffix(receivedKey)) + ")", null);
    ...
    /** Chỉ lộ 4 ký tự cuối của key khi log — tránh ghi thẳng khoá bí mật ra log file. */
    private static String safeSuffix(String key) {
    ...
    private boolean processSePayWebhook(String content, String code, int transferAmount, long transactionId) {
        ...
        if (!rewardCmds.isEmpty()) {
            ...
            Player p = Bukkit.getPlayerExact(matched.playerName);
            if (p != null && p.isOnline()) {
                // [Part 45] sendMessage đụng entity p → runForPlayer.
                SchedulerUtils.runForPlayer(plugin, p, () -> {
                    p.sendMessage(PayBotPlugin.f("§a§l[PayBot] §r§aĐã nhận thanh toán " + formatVnd(matched.amount) + " VND! Đang chờ admin cấu hình thưởng..."));
