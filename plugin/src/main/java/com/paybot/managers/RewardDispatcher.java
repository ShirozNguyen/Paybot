 ...
 * bị copy-paste riêng ở 5 nơi khác nhau (GuiListener, PayBotPlugin x2, ApproveCommand,
...
public final class RewardDispatcher {
     ...
     * - Đảm bảo giá trị >= 1.
     */
    public static int parseRewardMode(PayBotPlugin plugin, String type) {
     ...
     */
    public static List<String> resolveRewardCmds(PayBotPlugin plugin, int amount, String type) {
    ...
    public static String computeRewardAmt(PayBotPlugin plugin, int amount, String type) {
        String section = "bank".equalsIgnoreCase(type) ? "denom-rewards-bank" : "denom-rewards-card";
    ...
    public static boolean dispatchOrQueue(PayBotPlugin plugin, String rewardId, String playerName,
     ...
     */
    public static void deliverNow(PayBotPlugin plugin, Player target, List<String> rawCmds, String rewardAmt,
    ...
    private static void deliverPlayerEffectsSync(PayBotPlugin plugin, Player target, String rewardAmt,
                                                 String denomVnd, String type, String invoiceId, boolean isLate) {
        ...
        if (isBank) {
            target.sendMessage(PayBotPlugin.f("§a§l[PayBot] §r§aThanh toán thành công!" + lateSuffix));
            ...
            target.sendMessage(PayBotPlugin.f("§a[PayBot] §fThẻ của bạn đã được xử lý thành công!" + lateSuffix));
        }
        ...
        if (denomVnd != null && !denomVnd.isEmpty()) {
            try {
                ...
                Bukkit.getPluginManager().callEvent(new com.paybot.events.PayBotTopupEvent(
    ...
    public static void deliverNow(PayBotPlugin plugin, Player target, String singleRawCmd, String rewardAmt,
                                   String denomVnd, String type, String invoiceId, boolean isLate) {
