...
public class SePayApiClient {
    ...
    public SePayApiClient(PayBotPlugin plugin) {
        this.plugin = plugin;
    }
        ...
        try {
            ...
            if (code == 401 || code == 403) {
                ...
                if (consecutiveAuthFailures <= MAX_AUTH_FAILURES) {
                    NotificationManager.warn(plugin, "sepay-error", "[PayBot] SePay API: Token không hợp lệ hoặc không có quyền truy cập (HTTP " + code + ") [Lần " + consecutiveAuthFailures + "/" + MAX_AUTH_FAILURES + "].");
                    if (consecutiveAuthFailures == MAX_AUTH_FAILURES) {
                        NotificationManager.warn(plugin, "sepay-error", "[PayBot] SePay API: Đã đạt tối đa 5 lần thử thất bại Token SePay. Dừng polling cho tới khi token được cập nhật trong config hoặc reload plugin.");
                    }
                }
                result.error = "Token không hợp lệ hoặc không có quyền truy cập (HTTP " + code + ").";
                return result;
            }
            consecutiveAuthFailures = 0; // Reset khi thành công
