...
public class SmartConfigMerger {
     ...
     */
    public static int mergeWithComments(PayBotPlugin plugin, File liveFile) {
     ...
     * (PayBotPlugin.startConfigWatcher()) để tự đồng bộ config.yml định kỳ, không
     ...
     */
    public static int[] sync(PayBotPlugin plugin, File liveFile) {
        try (InputStream tplStream = plugin.getResource("config.yml")) {
            ...
            // ── REMOVE & RECOVER: xử lý block live (xoá rác & phục hồi comment bị mất) ──
            int removed = 0;
            int commentsRestored = 0;
            Map<String, Block> templateBlockMap = new HashMap<>();
            for (Block b : templateBlocks) if (b.topKey != null) templateBlockMap.put(b.topKey, b);
            ...
            for (Block b : liveBlocks) {
                ...
                // v5.4.3: Tự động khôi phục comment/note bị xóa trong block đã tồn tại
                if (b.topKey != null && templateBlockMap.containsKey(b.topKey)) {
                    Block tplBlock = templateBlockMap.get(b.topKey);
                    if (restoreComments(b, tplBlock)) {
                        commentsRestored++;
                        plugin.getLogger().info("[PayBot] SmartConfigMerger: khôi phục thành công comment/note bị xoá cho key '"
                                + b.topKey + "'.");
                    }
                }
            ...
            if (added > 0 || removed > 0 || commentsRestored > 0) {
    ...
    /** Tự động bổ sung các dòng comment/note bị xoá từ templateBlock vào liveBlock mà giữ nguyên giá trị cấu hình live. */
    private static boolean restoreComments(Block liveBlock, Block tplBlock) {
        if (liveBlock == null || tplBlock == null) return false;
        boolean modified = false;

        // 1. Phục hồi comment top-level trước key
        ...
        // 2. Phục hồi comment cho các sub-key
        for (int i = 0; i < tplBlock.lines.size(); i++) {
            ...
            if (tplLine.trim().startsWith("#") && i + 1 < tplBlock.lines.size()) {
                ...
                if (nextTplLine.startsWith(" ") && nextTplLine.contains(":")) {
                    String subKey = extractKey(nextTplLine);
                    if (!subKey.isEmpty()) {
                        for (int j = 0; j < liveBlock.lines.size(); j++) {
                            String liveLine = liveBlock.lines.get(j);
                            if (liveLine.startsWith(" ") && liveLine.contains(":") && extractKey(liveLine).equals(subKey)) {
