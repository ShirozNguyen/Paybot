...
public class DirectCardSubmitHandler {
     ...
     */
    public static String[] resolveCredentials(PayBotPlugin plugin, String site) {
    ...
    private static AttemptResult tryPostSubmit(PayBotPlugin plugin, String siteUrl, String partnerId, String partnerKey,
                                             String telco, int amount, String cardCode, String cardSerial, String requestId,
        ...
        try {
            ...
            com.paybot.utils.PayBotDebug.logSwallowed(plugin, "DirectCardSubmit POST tới " + siteUrl + ": " + reason, null);
            return new AttemptResult(null, reason);
        } catch (Throwable t) {
            com.paybot.utils.PayBotDebug.logSwallowed(plugin, "DirectCardSubmit POST tới " + siteUrl + " ném exception", t);
            return new AttemptResult(null, t.getClass().getSimpleName() + ": " + t.getMessage());
        }
    }

    private static AttemptResult tryGetSubmit(PayBotPlugin plugin, String siteUrl, String partnerId, String partnerKey,
                                            String telco, int amount, String cardCode, String cardSerial, String requestId,
                                            String sign, String userAgent) {
        try {
            ...
            String errBody = readErrorBodySafely(conn);
            String reason = "HTTP " + code + (errBody.isEmpty() ? "" : " — " + errBody);
            com.paybot.utils.PayBotDebug.logSwallowed(plugin, "DirectCardSubmit GET tới " + siteUrl + ": " + reason, null);
            return new AttemptResult(null, reason);
        } catch (Throwable t) {
            com.paybot.utils.PayBotDebug.logSwallowed(plugin, "DirectCardSubmit GET tới " + siteUrl + " ném exception", t);
            return new AttemptResult(null, t.getClass().getSimpleName() + ": " + t.getMessage());
        }
    }
