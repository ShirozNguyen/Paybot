...
public class BotHttpClient {
    ...
    public static void applyApiKey(HttpURLConnection conn, PayBotPlugin plugin) {
    ...
    public BotHttpClient(PayBotPlugin plugin) {
        this.plugin = plugin;
    }
        ...
        try {
            ...
            if (resp == null || !resp.has("ok") || !resp.get("ok").getAsBoolean()) {
                ...
                SchedulerUtils.runForPlayer(plugin, player, () ->
                    player.sendMessage(PayBotPlugin.f("§c[PayBot] §fLỗi tạo QR: " + msg)));
                return;
            }
            ...
            if (qrUrl.isEmpty() || invoiceId.isEmpty()) {
                SchedulerUtils.runForPlayer(plugin, player, () ->
                    player.sendMessage(PayBotPlugin.f("§c[PayBot] §fBot không trả về QR hợp lệ!")));
                return;
            }
            SchedulerUtils.runForPlayer(plugin, player, () ->
            ...
            SchedulerUtils.runForPlayer(plugin, player, () ->
                player.sendMessage(PayBotPlugin.f("§c[PayBot] §fKhông kết nối được tới bot!")));
        }
    }
     ...
     */
    private static SSLSocketFactory buildPinnedSocketFactory(PayBotPlugin plugin) throws Exception {
