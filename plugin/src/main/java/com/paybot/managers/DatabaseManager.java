...
public class DatabaseManager {
    ...
    // Circuit breaker chống "bão timeout" khi MySQL down kéo dài. TRƯỚC ĐÂY: mỗi lần conn bị
    // invalid, MỌI lần gọi tryConnectMySQL() đều thử full-connect lại (tối đa 20s/lần) — nếu
    // 1 caller lặp gọi hasBankOrder() nhiều lần liên tiếp (vd TransferContentGenerator thử tới
    // 100 mã ngẫu nhiên chống trùng), lý thuyết tối đa 100×20s ≈ 33 phút treo. Sau khi 1 lần
    // connect thất bại, ghi nhớ thời điểm — trong vòng RETRY_COOLDOWN_MS kế tiếp, trả false
    // NGAY LẬP TỨC mà không thử kết nối lại (không tốn thời gian mạng), thay vì lặp lại toàn bộ
    // logic connect+timeout mỗi lần. Cooldown ngắn (15s) để không trì hoãn quá lâu việc phát
    // hiện MySQL đã hồi phục trở lại.
    private volatile long lastMysqlFailTimeMs = 0L;
    private static final long MYSQL_RETRY_COOLDOWN_MS = 15_000L;

    public DatabaseManager(PayBotPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    private synchronized boolean tryConnectMySQL() {
        // Circuit breaker: nếu vừa thất bại gần đây (trong cooldown), trả false ngay — không
        // thử connect lại tốn thời gian. LƯU Ý: không dùng "conn == null" làm điều kiện phụ vì
        // conn KHÔNG bị reset về null khi connect thất bại (giữ nguyên giá trị cũ — có thể là 1
        // connection đã chết nếu từng kết nối được rồi mất, không chỉ null lúc chưa từng kết nối
        // lần nào) — dựa thuần vào mốc thời gian thất bại gần nhất là đúng nguyên lý circuit
        // breaker, không phụ thuộc trạng thái conn. Trường hợp conn ĐANG valid thật sự (mới hồi
        // phục ở nơi khác) hiếm khi trùng đúng lúc trong cooldown 15s; nếu có, chỉ trì hoãn phát
        // hiện hồi phục tối đa 15s — đánh đổi chấp nhận được so với rủi ro treo tới 33 phút.
        ...
        try {
            ...
            // v5.5.5 Part 58: cập nhật circuit breaker dựa trên kết quả THẬT (result đã gán ở
            // nhánh try trước khi return; các nhánh catch không gán lại result nên vẫn là false
            // mặc định — đúng ý muốn vì mọi nhánh catch đều là thất bại). Không đặt trong nhánh
            // try/catch riêng lẻ để tránh sót nhánh nào — finally luôn chạy dù return ở đâu.
            if (result) {
    ...
    public synchronized boolean hasRewardDelivery(String paymentType, String paymentReference, String rewardHash) {
        ...
        try (PreparedStatement ps = c.prepareStatement(sql)) {
            ...
            plugin.getLogger().warning("[DB] hasRewardDelivery error: " + e.getMessage());
            return false;
        }
    }
    ...
    public synchronized boolean recordRewardDelivery(String deliveryId, String paymentType, String paymentReference,
        ...
        try (PreparedStatement ps = c.prepareStatement(sql)) {
            ...
            plugin.getLogger().warning("[DB] recordRewardDelivery error: " + e.getMessage());
            return false;
        }
    }
    ...
    public synchronized boolean claimBankOrder(String invoiceId, String expectedStatus, String nextStatus) {
        ...
        try (PreparedStatement ps = c.prepareStatement(sql)) {
            ...
            plugin.getLogger().warning("[DB] claimBankOrder error: " + e.getMessage());
            return false;
        }
    }
    ...
    public synchronized boolean claimCardOrder(String requestId, String expectedStatus, String nextStatus) {
        ...
        try (PreparedStatement ps = c.prepareStatement(sql)) {
            ...
            plugin.getLogger().warning("[DB] claimCardOrder error: " + e.getMessage());
            return false;
        }
    }
    ...
    public synchronized boolean claimOfflineReward(String rewardId) {
        ...
        try (PreparedStatement ps = c.prepareStatement(sql)) {
            ...
            plugin.getLogger().warning("[DB] claimOfflineReward error: " + e.getMessage());
            return false;
        }
    }
    ...
    public synchronized void failOfflineReward(String rewardId) {
        ...
        try (PreparedStatement ps = c.prepareStatement(sql)) {
            ...
            plugin.getLogger().warning("[DB] failOfflineReward error: " + e.getMessage());
        }
    }
    ...
    public synchronized int revertProcessingOfflineRewards() {
        ...
        try (Statement st = c.createStatement()) {
            ...
            plugin.getLogger().warning("[DB] revertProcessingOfflineRewards error: " + e.getMessage());
            return 0;
        }
    }
}
