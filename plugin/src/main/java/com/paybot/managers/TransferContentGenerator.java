...
public final class TransferContentGenerator {
     ...
     */
    public static void validateConfig(PayBotPlugin plugin) {
    ...
    public static String generate(PayBotPlugin plugin) {
        ...
        boolean includeDate    = plugin.getConfig().getBoolean("transfer-content.include-date-prefix", false);
        boolean checkDuplicate = plugin.getConfig().getBoolean("transfer-content.check-duplicate-orders", true);
        int     length         = plugin.getConfig().getInt("transfer-content.code-length", 12);
        int     mode           = plugin.getConfig().getInt("transfer-content.code-mode", 0);
        ...
        for (int attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            boolean existsInCache = plugin.getLocalOrderManager() != null && plugin.getLocalOrderManager().getBankOrder(candidate) != null;
            boolean existsInDb    = plugin.getDatabaseManager() != null && plugin.getDatabaseManager().hasBankOrder(candidate);
        ...
        // Fallback an toàn tuyệt đối nếu thử 100 lần vẫn vấp trùng (hiếm)
