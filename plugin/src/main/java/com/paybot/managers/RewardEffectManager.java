...
public class RewardEffectManager {
     ...
     */
    public static void trigger(PayBotPlugin plugin, Player player, int amount) {
        ...
        if (sound) {
            if (amount >= 100_000) {
                ...
                // [Part 45] playSound đụng entity player → runForPlayerLater, không runSyncLater.
                SchedulerUtils.runForPlayerLater(plugin, player, () -> {
                    if (player.isOnline())
        ...
        if (firework) {
            ...
            // [Part 45] spawnFirework spawn entity tại location player → runForPlayerLater.
            SchedulerUtils.runForPlayerLater(plugin, player, () -> {
    ...
    private static void spawnFirework(PayBotPlugin plugin, Player player) {
        if (player == null || !player.isOnline()) return;
        try {
     ...
     */
    public static void notifyPaymentReceived(PayBotPlugin plugin, Player player, int amount) {
        if (player == null || !player.isOnline()) return;
        boolean notification = plugin.getConfig().getBoolean("Reward_Notification", true);
        if (!notification) return;
        // [Part 45] sendTitle đụng entity player → tự dispatch, không giả định caller
        // đã ở đúng thread (audit phát hiện cả 3 nơi gọi hiện tại đều gọi qua runSync/Global).
