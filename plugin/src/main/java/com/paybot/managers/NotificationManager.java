...
public final class NotificationManager {
     ...
     */
    private static int getMaxPerMinute(PayBotPlugin plugin) {
    ...
    private static boolean allowRate(PayBotPlugin plugin, String key) {
    ...
    public static boolean enabled(PayBotPlugin plugin, String key) {
    ...
    public static void log(PayBotPlugin plugin, String key, String message) {
    ...
    public static void warn(PayBotPlugin plugin, String key, String message) {
    ...
    public static void warn(PayBotPlugin plugin, String key, String message, Throwable t) {
     ...
     */
    public static void notifyAdmins(PayBotPlugin plugin, String key, String message) {
        if (!enabled(plugin, key) || !allowRate(plugin, key)) return;
        // [Part 45] getOnlinePlayers() KHÔNG an toàn gọi từ thread async trần (Javadoc
        // Bukkit chính thức: "Any use of this collection from asynchronous threads is
        // unsafe") — đọc danh sách trên runSync, rồi dispatch RIÊNG từng player qua
        // entity-scheduler của họ để gửi tin (broadcastMessage không đảm bảo tới hết
        // mọi player trên Folia — GitHub issue #382, PaperMC/Folia).
    ...
    public static void broadcast(PayBotPlugin plugin, String key, String message) {
        if (!enabled(plugin, key) || !allowRate(plugin, key)) return;
        // [Part 45] Xem lưu ý ở notifyAdmins().
        SchedulerUtils.runSync(plugin, () -> {
            for (Player p : Bukkit.getOnlinePlayers()) {
                SchedulerUtils.runForPlayer(plugin, p, () -> p.sendMessage(message));
            }
