...
import com.paybot.commands.NapBankCommand;
...
import static com.paybot.gui.GuiUtil.*;
...
public final class GuiListener implements Listener {
    ...
    public GuiListener(PayBotPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        ...
        if (title.startsWith("§6§lChọn mệnh giá - §e")) {
            ...
            if (denom != null) {
                ...
                player.closeInventory();
                player.sendMessage(PayBotPlugin.f("§6[PayBot] §eNhập §bmã thẻ §evào chat:"));
        ...
        if (title.equals("§6§lChỉnh sửa mệnh giá nạp")) {
            ...
            if (info != null) {
                ...
                player.closeInventory();
                player.sendMessage(PayBotPlugin.f("§6[PayBot] §eCấu hình mệnh giá §b"
                        + formatDenom(info.denom()) + " §e(" + info.type() + ")"));
                player.sendMessage(PayBotPlugin.f("§6Nhập §blệnh thưởng #1 §e(không cần dấu /):  §7§o(cancel để huỷ)"));
        ...
        if (title.startsWith("§6§lDanh sách đơn")) {
            ...
            if (order != null) {
                ...
                // Refresh GUI
                SchedulerUtils.runForPlayerLater(plugin, player, () -> TopupListGui.open(player, plugin, page, filter), 2L);
            }
            return;
        }
    }
    ...
    public void onPlayerChat(AsyncPlayerChatEvent event) {
        ...
        if (input.equalsIgnoreCase("cancel")) {
            GuiSession.clear(player.getUniqueId());
            player.sendMessage(PayBotPlugin.f("§c[PayBot] §fĐã huỷ."));
            return;
        }
        ...
        switch (s.stage) {
            ...
            case CARD_WAIT_CODE -> {
                if (!input.matches("[0-9]+")) {
                    player.sendMessage(PayBotPlugin.f("§c[PayBot] §fMã thẻ chỉ chứa số! Nhập lại:")); return;
                }
                s.cardCode = input;   // v4.3.0: lưu mã thẻ, chuyển sang hỏi serial
                s.stage    = GuiSession.Stage.CARD_WAIT_SERIAL;
                player.sendMessage(PayBotPlugin.f("§7[PayBot] §fĐã nhận mã thẻ: §b" + maskFull(input)));
                player.sendMessage(PayBotPlugin.f("§6[PayBot] §eNhập §bserial §ethẻ vào chat:  §7§o(cancel để huỷ)"));
            }

            // ── Card: serial (bước 2) ───────────────────────────────────────────
            case CARD_WAIT_SERIAL -> {
                if (!input.matches("[0-9]+")) {
                    player.sendMessage(PayBotPlugin.f("§c[PayBot] §fSerial chỉ chứa số! Nhập lại:")); return;
                }
                ...
                GuiSession.clear(player.getUniqueId());
                // Echo masked
                player.sendMessage(PayBotPlugin.f("§7[PayBot] §fĐã nhận serial: §b" + maskPartial(serial)));
                SchedulerUtils.runForPlayer(plugin, player, () -> {
                    plugin.getCardManager().setPending(player.getName(),
                            new com.paybot.managers.CardManager.PendingCard(
                                    player.getName(), telco, denom, code, serial));
                    player.sendMessage(PayBotPlugin.f("§e[PayBot] §fXem lại trước khi xác nhận:"));
            ...
            case EDIT_WAIT_CMD_CARD, EDIT_WAIT_CMD_BANK -> {
                ...
                if (!isFirst && input.equalsIgnoreCase("skip")) {
                    ...
                    player.sendMessage(PayBotPlugin.f("§7[PayBot] §fĐã dừng — tổng §a"
                            + s.editCmdList.size() + " §flệnh thưởng."));
                    player.sendMessage(PayBotPlugin.f("§6[PayBot] §eNhập §bsố lượng item thưởng khi nạp §e(số nguyên):  §7§o(cancel để huỷ)"));
                    return;
                }
                s.editCmdList.add(input);
                player.sendMessage(PayBotPlugin.f("§7[PayBot] §fLệnh #" + s.editCmdList.size() + " đã nhận: §a" + input));
                if (s.editCmdList.size() >= com.paybot.managers.RewardDispatcher.MAX_CMDS) {
                    s.stage = (s.stage == GuiSession.Stage.EDIT_WAIT_CMD_CARD)
                            ? GuiSession.Stage.EDIT_WAIT_AMT_CARD
                            : GuiSession.Stage.EDIT_WAIT_AMT_BANK;
                    player.sendMessage(PayBotPlugin.f("§7[PayBot] §fĐã đạt tối đa §e"
                            + com.paybot.managers.RewardDispatcher.MAX_CMDS + " §flệnh."));
                    player.sendMessage(PayBotPlugin.f("§6[PayBot] §eNhập §bsố lượng item thưởng khi nạp §e(số nguyên):  §7§o(cancel để huỷ)"));
                } else {
                    player.sendMessage(PayBotPlugin.f("§6[PayBot] §eNhập §blệnh thưởng #" + (s.editCmdList.size() + 1)
            ...
            case EDIT_WAIT_AMT_CARD, EDIT_WAIT_AMT_BANK -> {
                try { Integer.parseInt(input); }
                    ...
                    player.sendMessage(PayBotPlugin.f("§c[PayBot] §fPhải nhập số nguyên! Nhập lại:")); return;
                }
                ...
                SchedulerUtils.runForPlayer(plugin, player, () -> {
                    ...
                    player.sendMessage(PayBotPlugin.f("§a[PayBot] §fĐã lưu §e" + formatDenom(denom)
    ...
    private void handleApproveOrder(Player admin, TopupListGui.OrderEntry order) {
        try {
            ...
            admin.sendMessage(PayBotPlugin.f("§c[PayBot] §fDuyệt đơn lỗi! Xem console để biết thêm."));
        }
    }
    ...
    private void handleApproveOrderInternal(Player admin, TopupListGui.OrderEntry order) {
        ...
        if ("card".equals(order.type())) {
            if (LocalOrderManager.CARD_APPROVED.equals(order.status())) {
                admin.sendMessage(PayBotPlugin.f("§c[PayBot] §fĐơn này đã được duyệt rồi.")); return;
            }
            ...
            if (!LocalOrderManager.CARD_SUCCESS.equals(order.status())) {
                admin.sendMessage(PayBotPlugin.f("§c[PayBot] §fĐơn này không ở trạng thái có thể duyệt "
                        + "(hiện tại: §e" + com.paybot.log.LogManager.cardStatusLabel(order.status()) + "§f)."));
                return;
            }
            List<String> rewardCmds = RewardDispatcher.resolveRewardCmds(plugin, order.amount(), "card");
            if (rewardCmds.isEmpty()) {
                admin.sendMessage(PayBotPlugin.f("§c[PayBot] §fChưa cấu hình lệnh thưởng cho mệnh giá §e"
            ...
            admin.sendMessage(PayBotPlugin.f("§a[PayBot] §fĐã duyệt đơn thẻ §e" + order.playerName()
            ...
            if (!LocalOrderManager.BANK_PAID.equals(order.status())) {
                admin.sendMessage(PayBotPlugin.f("§c[PayBot] §fĐơn này không ở trạng thái có thể duyệt "
                        + "(hiện tại: §e" + com.paybot.log.LogManager.bankStatusLabel(order.status()) + "§f)."));
                return;
            }
            ...
            admin.sendMessage(PayBotPlugin.f("§a[PayBot] §fĐã duyệt đơn bank §e" + order.playerName()
                    + " §f- §b" + formatVnd(order.amount()) + " VND"
                    + (wasOnline ? "" : " §7(player offline — sẽ nhận khi vào lại)")));
        }
    }
    ...
    // ══════════════════════════════════════════════════════════════════════════
    public static void simulateBankSuccess(PayBotPlugin plugin, Player player, int amount) {
        List<String> cmds = RewardDispatcher.resolveRewardCmds(plugin, amount, "bank");
        if (cmds.isEmpty()) {
            player.sendMessage(PayBotPlugin.f("§c[TEST] §fChưa cấu hình lệnh thưởng cho mệnh giá §e"
        ...
        player.sendMessage(PayBotPlugin.f("§e§l[TEST] §r§eĐã giả lập đơn nạp bank §b" + formatVnd(amount)
    ...
    public static void simulateCardSuccess(PayBotPlugin plugin, Player player, String telco, int amount) {
        ...
        player.sendMessage(PayBotPlugin.f("§e§l[TEST] §r§eĐã giả lập thẻ §b" + telco + " " + formatVnd(amount)
                + " VND §ethành công cho §b" + player.getName() + "§e!"));
